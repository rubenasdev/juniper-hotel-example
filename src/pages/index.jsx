...
const Icon=({name,size=20})=>{const d={right:'M5 12h14m-5-5 5 5-5 5',left:'M19 12H5m5 5-5-5 5-5',plus:'M12 5v14M5 12h14',minus:'M5 12h14',calendar:'M5 4v3m14-3v3M4 9h16v11H4z',down:'m7 10 5 5 5-5',pin:'M20 10c0 5-8 11-8 11S4 15 4 10a8 8 0 1 1 16 0zm-8-3a3 3 0 1 0 0 6 3 3 0 0 0 0-6',bed:'M3 18v-7h18v7M5 11V6h6v5m0 0V7h6a2 2 0 0 1 2 2v2M3 15h18M5 18v3m14-3v3',users:'M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2m7-10a4 4 0 1 0 0-8 4 4 0 0 0 0 8m13 10v-2a4 4 0 0 0-3-3.87m-2-11.96a4 4 0 0 1 0 7.75',expand:'M8 3H3v5m13-5h5v5M8 21H3v-5m13 5h5v-5M3 8l6-5m12 5-6-5M3 16l6 5m12-5-6 5',bath:'M4 12h16v3a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4v-3Zm3 0V6a3 3 0 0 1 6 0v1m-7 12-1 2m13-2 1 2',star:'m12 3 2.7 5.47 6.03.88-4.36 4.25 1.03 6-5.39-2.83L6.97 18.6 8 12.6 3.64 8.35l6.03-.88L12 3Z',home:'M3 10.5 12 3l9 7.5V21H3V10.5Zm5 10v-7h8v7',terrace:'M3 13h18M5 13V8h14v5M7 8V5h10v3M6 13v7m12-7v7',coffee:'M4 8h12v6a5 5 0 0 1-5 5H9a5 5 0 0 1-5-5V8Zm12 2h2a3 3 0 0 1 0 6h-3M6 4h8',wifi:'M5 12.5a10 10 0 0 1 14 0M8.5 16a5 5 0 0 1 7 0M12 20h.01',snow:'M12 2v20M4.2 6.5l15.6 11M19.8 6.5l-15.6 11M9 3.8l3 2 3-2M9 20.2l3-2 3 2',shield:'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10Zm-3-10 2 2 4-4'}[name];return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={d}/></svg>};
...
export function Suites({go}){
  ...
  return <section className="suites">
    ...
    {imageOpen&&<div className="suite-lightbox" role="dialog" aria-modal="true" aria-label={`Imagem ampliada da ${s.name}`} onMouseDown={event=>{if(event.target===event.currentTarget)setImageOpen(false)}}><button className="suite-lightbox-close" onClick={()=>setImageOpen(false)} aria-label="Fechar imagem"><span>Fechar</span><Icon name="plus" size={22}/></button><img src={s.photos[photo]} alt={`${s.name} — fotografia ${photo+1} ampliada`}/><div className="suite-lightbox-count">0{photo+1} / 0{s.photos.length}</div></div>}
...
function DiningVideoLoop(){
  ...
  return <div className="dining-video-loop" aria-hidden="true">{diningVideos.map((src,index)=><video key={src} ref={element=>videoRefs.current[index]=element} className={active===index?'active':''} autoPlay={index===0} muted playsInline preload={index===0?'auto':'metadata'} onEnded={()=>{if(active===index)setActive((index+1)%diningVideos.length)}}><source src={src} type="video/mp4"/></video>)}<div className="dining-video-mask"/><span className="dining-clip-count">0{active+1} / 03</span></div>
}
...
export function ExperienceDetail({go,slug}){
  ...
  return <article ref={root} className={`experience-detail experience--${experience.slug}`}>
    <section className="experience-detail-hero" style={{backgroundImage:`url(${experience.hero})`}}><video key={experience.slug} autoPlay muted loop playsInline preload="metadata" poster={experience.hero} aria-hidden="true"><source src={experienceVideos[experience.slug]} type="video/mp4"/></video><div className="experience-hero-copy"><button onClick={()=>go('/experiences')}><ArrowLeft/> Todas as experiências</button><small>{experience.kicker}</small><h1>{experience.title}</h1><p>{experience.lead}</p></div><span className="experience-scroll">Deslizar <i/></span></section>
    ...
    {lightbox!==null&&<div className="experience-lightbox" role="dialog" aria-modal="true" aria-label="Galeria da experiência" onClick={()=>setLightbox(null)}><button className="close" onClick={()=>setLightbox(null)} aria-label="Fechar"><Plus/></button><button className="prev" onClick={e=>{e.stopPropagation();setLightbox((lightbox+3)%4)}}><ArrowLeft/></button><img src={media.gallery[lightbox]} alt={`${experience.title}, imagem ${lightbox+1}`} onClick={e=>e.stopPropagation()}/><button className="next" onClick={e=>{e.stopPropagation();setLightbox((lightbox+1)%4)}}><ArrowRight/></button><span>0{lightbox+1} / 04</span></div>}
  </article>
}
...
export function Booking(){
  ...
  return <section className="booking"><div className="bookingform">{done?<div className="success"><small>Pedido recebido</small><h1>O seu tempo<br/>começa aqui.</h1><p>Recebemos o seu pedido para {formatBookingDate(start)} — {formatBookingDate(end)}, em {selectedRoom}{selectedExperiences.length?`, incluindo ${selectedExperiences.length} ${selectedExperiences.length===1?'experiência':'experiências'}`:''}. A nossa equipa entrará em contacto consigo muito em breve.</p><button onClick={()=>setDone(false)}>Alterar pedido</button></div>:<><h1>Reservas</h1><p>Comece a desenhar a sua estadia e acrescente os momentos que quer viver.</p><form onSubmit={e=>{e.preventDefault();setDone(true)}}><label className="date-field">Check-in — Check-out<button type="button" className="date-trigger" onClick={()=>setCalendarOpen(open=>!open)} aria-expanded={calendarOpen}><span>{formatBookingDate(start)} — {formatBookingDate(end)}</span><CalendarDays size={17}/></button>{calendarOpen&&<BookingCalendar start={start} end={end} onChange={(nextStart,nextEnd)=>{setStart(nextStart);setEnd(nextEnd)}} onClose={()=>setCalendarOpen(false)}/>}</label><label>Hóspedes<span className="step"><button type="button" onClick={()=>setG(Math.max(1,g-1))}><Minus/></button><b>{g} Adultos</b><button type="button" onClick={()=>setG(g+1)}><Plus/></button></span></label><fieldset className="booking-rooms"><legend>Alojamento <small>Selecione um</small></legend><div>{bookingRooms.map(room=><button type="button" key={room.name} className={selectedRoom===room.name?'selected':''} onMouseEnter={()=>showVisual(room)} onMouseLeave={()=>setHoveredVisual(null)} onFocus={()=>showVisual(room)} onBlur={()=>setHoveredVisual(null)} onClick={()=>selectRoom(room)} aria-pressed={selectedRoom===room.name}><span><b>{room.name}</b><small>{room.meta}</small></span><em><small>Desde</small>{room.price}<i>/ noite</i></em></button>)}</div></fieldset><fieldset className="booking-experiences"><legend>Experiências <small>Selecione todas as que desejar</small></legend><div>{bookingExperiences.map(experience=><button type="button" key={experience.name} className={selectedExperiences.includes(experience.name)?'selected':''} onMouseEnter={()=>showVisual(experience)} onMouseLeave={()=>setHoveredVisual(null)} onFocus={()=>showVisual(experience)} onBlur={()=>setHoveredVisual(null)} onClick={()=>toggleExperience(experience)} aria-pressed={selectedExperiences.includes(experience.name)}><span><b>{experience.name}</b><small>{experience.meta}</small></span><em><small>Por pessoa</small>{experience.price}</em><i>{selectedExperiences.includes(experience.name)?'✓':'+'}</i></button>)}</div></fieldset><button className="submit" disabled={!start||!end}>Verificar disponibilidade</button></form><small>Prefere falar connosco?<br/>+351 265 499 120</small></>}</div><figure className="booking-visual"><video className="booking-base-video" autoPlay muted loop playsInline><source src="/media/08-evening.mp4"/></video>{activeVisual.video?<video className="booking-active-media" key={`${activeVisual.name}-${visualVersion}`} autoPlay muted loop playsInline poster={activeVisual.image} aria-label={activeVisual.name}><source src={activeVisual.video} type="video/mp4"/></video>:<img className="booking-active-media" key={`${activeVisual.name}-${visualVersion}`} src={activeVisual.image} alt={activeVisual.name}/>}<div className="booking-visual-caption"><span>{hoveredVisual?'A descobrir':selectedExperienceData?'Experiência selecionada':'Alojamento selecionado'}</span><b>{activeVisual.name}</b><small>{activeVisual.price}</small></div><figcaption>Comporta · Portugal</figcaption></figure></section>}
