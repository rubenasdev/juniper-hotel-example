    ...
    <section className="home-rooms" id="home-rooms" ref={section}>
      ...
      <div className="home-room-rail" ref={rail}>
        {rooms.map((room,index)=><button key={room.name} onClick={()=>openRoom(index)} className="home-room-card">
          <img src={room.image} alt={room.name}/><span>0{index+1}</span><div><h3>{room.name}</h3><p>{room.meta}</p><i><Icon name="right"/></i></div>
        </button>)}
        <button className="home-room-end" onClick={()=>go('/suites')}><span>Ver todos<br/>os espaços</span><Icon name="right"/></button>
      </div>
      <div className="rail-instruction"><i/><span>Scroll horizontal</span></div>
    </section>

    <section className="home-spa" id="home-spa">
      <div className="spa-copy"><span>03 / Spa</span><h2>O corpo também<br/>precisa de <em>silêncio.</em></h2><p>Rituais lentos, botânica local e o tempo necessário para regressar a si.</p><a href="/experiences" onClick={event=>{event.preventDefault();go('/experiences')}}>Descobrir os rituais <Icon name="right"/></a></div>
      <div className="spa-media"><MediaVideo src="/media/07-retreat.mp4" loop/><span>Respirar · Libertar · Renovar</span></div>
      <blockquote>“O luxo de não ter pressa.”</blockquote>
    </section>

    <section className="home-table" id="home-table">
      <div className="table-media"><MediaVideo src="/media/05-dining.mp4" loop/></div>
      <div className="table-copy"><span>04 / Mesa</span><h2>Da terra,<br/>para a mesa.</h2><p>Uma cozinha guiada pelo que cresce perto, pelo que chega do mar e pelo ritmo de cada estação.</p><div><a href="/dining" onClick={event=>{event.preventDefault();go('/dining')}}>Conhecer o restaurante <Icon name="right"/></a><small>Jantar · 19:00—23:00</small></div></div>
